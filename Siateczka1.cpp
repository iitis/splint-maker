...
void CSiateczka1::zrobWycisk(std::shared_ptr<CModel3D> o)
{
...
//	FILE * plik = fopen("siateczka_wezly.txt", "w");
	...
	{
		UI::STATUSBAR::printfTimed( 1000, L"Tworzę wycisk. Y=:%d", iy);
		...
		for (int ix = sMinX; ix <= sMaxX; ix++)
		{
			...
			// przeliczam wierzcho�ek ze wsp�rz�dnych "rzutni" do wsp�rz�dnych okna
			CTriple<double> t0 = obj->getTransform().l2w( CTriple<double>( x, y, (double)pMax.Z() ) );
			CTriple<double> t1 = obj->getTransform().l2w( CTriple<double>( x, y, (double)pMin.Z() ) );

			double odl = testRay( t0, t1, o);

			if ( odl < CVector3d( t0, t1 ).length() )
			{
				if (siateczka.end() == siateczka.find(std::pair<int, int>(ix, iy)))
				{
					size_t index = vIndex( ix, iy );
					siateczka[std::pair<int, int>(ix, iy)] = &rzutnia->vertices()[index];
				}
				siateczka[std::pair<int, int>(ix, iy)]->Z(pMax.Z() - odl);

//				fprintf(plik, "%d, %d : %f\n", ix, iy, siateczka[std::pair<int, int>(ix, iy)]->Z());
			}
		}
	}
//	fclose(plik);
...
//void CSiateczka1::zrobWyciskSumy(CModel3D *o1, CModel3D *o2)
//{
//	unsigned long t1 = GetTickCount();
//
//	CMesh * zeby1 = (CMesh*)o1->getData();
//	CMesh * zeby2 = (CMesh*)o2->getData();
//
//	CMesh * rzutnia = (CMesh*)obj->getData();
//
//	for (int i = 0; i < zeby1->faces().size(); i++)
//	{
//		UI::STATUSBAR::printfTimed(1000, L"Tworzę wycisk 1. Zosta�o %d �cian.", i);
//
//		CFace f = zeby1->faces().at(i);
//
//		CVertex a = zeby1->vertices().at(f.A());
//		CVertex b = zeby1->vertices().at(f.B());
//		CVertex c = zeby1->vertices().at(f.C());
//
...
//
//		CFace f = zeby2->faces().at(i);
//
//		CVertex a = zeby2->vertices().at(f.A());
//		CVertex b = zeby2->vertices().at(f.B());
//		CVertex c = zeby2->vertices().at(f.C());
//
//		int rMinX = std::max((int)floor(div * MIN3(a.X(), b.X(), c.X()) - 1.0), sMinX);
//		int rMinY = std::max((int)floor(div * MIN3(a.Y(), b.Y(), c.Y()) - 1.0), sMinY);
//
...
//
//void CSiateczka1::zrobWyciskSumy1111(CModel3D* o1)
//{
//	unsigned long t1 = GetTickCount();
//
//	CMesh* rzutnia = (CMesh*)obj->getData();
...
//		UI::STATUSBAR::printfTimed(1000, L"Tworzę wycisk 1. Zostało %d ścian.", i);
//
//		CTriangle t = CTriangle(i, zeby1).transformByMatrix(trans10.toEigenMatrix4d());
//
//		CPoint3d tmin = t.getBoundingBox().getMin();
//		CPoint3d tmax = t.getBoundingBox().getMax();
//
...
//					if (siateczka.end() != siateczka.find(std::pair<int, int>(ix, iy)))
//					{
//						double dist = siateczka[std::pair<int, int>(ix, iy)]->z - pIP.z;
//						if (dist < 0.1)
//						{
//							siateczka[std::pair<int, int>(ix, iy)]->z = pIP.z;
//						}
...
//
//void CSiateczka1::zrobWyciskSumy4_aaa(CModel3D* o1, CModel3D* o2, bool test2)
//{
//	unsigned long t1 = GetTickCount();
//
//	CMesh* rzutnia = (CMesh*)obj->getData();
...
//		UI::STATUSBAR::printfTimed(1000, L"Tworzę wycisk 2. Zostało %d ścian.", i);
//
//		CTriangle t = CTriangle(i, zeby2).transformByMatrix(trans20.toEigenMatrix4d());
//
//		CPoint3d tmin = t.getBoundingBox().getMin();
//		CPoint3d tmax = t.getBoundingBox().getMax();
//
...
//					else
//					{
//						float& siat_Z = siateczka[std::pair<int, int>(ix, iy)]->z;
//						double pip_Z = pIP.z;
//
//						if ((pip_Z - siat_Z) < 0.3)
//						{
...
// o1 - to jest szczęka rozepchana do zewnętrznej powierzchni
// o2 - to jest powierzchnia okluzji w pozycji terapeutycznej
// o3 - to jest szczęka rozepchana do wewnętrznej powierzchni
//void CSiateczka1::zrobWyciskSumy5(CModel3D* o1, CModel3D* o2, CModel3D* o3, std::set<CVertex> *bledy)
//{
//	unsigned long t1 = GetTickCount();
//
//	CMesh* rzutnia = (CMesh*)obj->getData();
...
//		}
//
//		CMesh* zeby3 = (CMesh*)o3->getData();
//
//		zeby3->setLabel("wersja_5");
//
//		CTransform trans30 = CTransform::fromTo(o3->getTransform(), obj->getTransform());
//
//		// SIATKA OKLUZJI
//		for (int i = 0; i < zeby3->faces().size(); i++)
//		{
//			UI::STATUSBAR::printfTimed(1000, L"Tworzę wycisk 3. Zostało %d ścian.", i);
//
//			CTriangle t = CTriangle(i, zeby3).transformByMatrix(trans30.toEigenMatrix4d());
//
//			CPoint3d tmin = t.getBoundingBox().getMin();
//			CPoint3d tmax = t.getBoundingBox().getMax();
//
...
//
//							if (bledy) {
//								bledy->insert(*siateczka[std::pair<int, int>(ix, iy)]);
//							}
//						}
//					}
//				}
...
// o4 - to jest zuchwa
//
//void CSiateczka1::zrobWyciskSumy6(CModel3D* o1, CModel3D* o2, CModel3D* o3, CModel3D* o4, std::set<CVertex>* bledy)
//{
//	unsigned long t1 = GetTickCount();
//
//	CMesh* rzutnia = (CMesh*)obj->getData();
...
//	}
//
//	UI::STATUSBAR::printf(L"Wycisk 2 gotowy");
//
//	CMesh* zeby3 = (CMesh*)o3->getData();
//
//	zeby3->setLabel("wersja_5");
...
//	}
//
//	UI::STATUSBAR::printf(L"Wycisk 3 gotowy");
//
//	//qInfo() << o4->children().size();
//
//	CMesh* zeby4 = (CMesh*)o4->getData();
//
//	//CTransform trans40 = CTransform::fromTo(zeby4->getGlobalTransformationMatrix(), obj->getGlobalTransformationMatrix());
//	//Eigen::Matrix4d M = obj->getGlobalTransformationMatrix().inverse() * zeby4->getGlobalTransformationMatrix();
//
...
//
//	KDNode2* tree = KDNode2::build(zeby4);
//	KDNode2::HitMap hmap;
//
//	rzutnia->calcVN();
//
...
//		UI::STATUSBAR::printfTimed(1000, L"Tworzę wycisk 4. Wertex %d z %d.", i, rzutnia->vertices().size());
//		
...
//
//		hmap.clear();
//		bool hit = tree->hit(zeby4, p0, -vr, hmap);
//
//		if (hit) {
...
//		//while (ptr != nullptr)
//		//{
//		//	for (const INDEX_TYPE& j : ptr->m_tris)
//		//	{
//		//		CTriangle tr(j, zeby4);
//		//		//CVector3d vf = tr.getNormal();
//		//		CPoint3d p1 = tr.getClosestPoint(p0);
//		//		
//		//		CVector3d v = CVector3d(p0, p1);
//		//		double dist = v.length();
//		//		v.normalize();
//
//		//		CVector3d vf = CVector3d(mid, p1).getNormalized();
//
//		//		// bedzie dodatni gdy scianki sa w te sama strone czyli jest przeciecie
//		//		double dir = v.dotProduct(vf);
//		//		//double dir = vr.dotProduct(v);
//
//		//		if ((dir > 0.7) && (dist < minDist)) {
//		//			minDist = dist;
//		//			minDir = dir;
//		//			minV = v;
//		//		}
...
//		//	//if (found) break;
//
//		//	if (ptr->m_left != nullptr) ptr = ptr->m_left;
//		//	else if ((ptr->m_right != nullptr) && ptr->m_right->m_bbox.contains(p0)) ptr = ptr->m_right;
//		//	else ptr = nullptr;
//		//	//if ((ptr->m_left != nullptr) && ptr->m_left->m_bbox.contains(p0)) ptr = ptr->m_left;
//		//	//else if ((ptr->m_right != nullptr) && ptr->m_right->m_bbox.contains(p0)) ptr = ptr->m_right;
//		//	//else ptr = nullptr;
//		//}
//
//		//if (minDist < 1.0) {
//		//	qInfo() << "i = " << i << " dist = " << minDist;
//
//		//	//found = true;
//		//	//break;
//		//	if (bledy) {
...
//	//zeby4
//
//	//for (int i = 0; i < zeby4->faces().size(); i++)
//	//{
//	//	UI::STATUSBAR::printfTimed(1000, L"Tworzę wycisk 4. Zostało %d ścian.", i);
//
//	//	CTriangle t = CTriangle(i, zeby4);// .transformByMatrix(M);
//
//	//	CPoint3d tmin = t.getBoundingBox().getMin();
//	//	CPoint3d tmax = t.getBoundingBox().getMax();
//
...
//	//					//siateczka[std::pair<int, int>(ix, iy)]->z = pIP.z;
//	//				}
//	//				else if (siateczka[std::pair<int, int>(ix, iy)]->z >= (pIP.z - 2.0))
//	//				{
//	//					//siateczka[std::pair<int, int>(ix, iy)]->z = 0.0;// pIP.z;
//
//	//					if (bledy) {
//	//						bledy->insert(*siateczka[std::pair<int, int>(ix, iy)]);
//	//					}
...
// o4 - to jest zuchwa
//void CSiateczka1::zrobWyciskZuchwy(CModel3D* o4, double ndir, std::set<CVertex>* bledy, std::set<int>* bledy2)
//{
//	unsigned long t1 = GetTickCount();
//
//	CMesh* zeby4 = (CMesh*)o4->getData();
...
//
//		hmap.clear();
//		bool hit = tree->hit(zeby4, p0, vr*ndir, hmap);
//
//		if (hit) {
//			//qInfo() << "hmap size = " << hmap.size();
//
...
//
//void CSiateczka1::zrobWyciskSumy3(CModel3D* o1, CModel3D* o2)
//{
//	unsigned long t1 = GetTickCount();
//
//	CMesh* zeby1 = (CMesh*)o1->getData();
...
//		CFace f = zeby1->faces().at(i);
//
//		CVertex za = zeby1->vertices().at(f.A());
//		CVertex zb = zeby1->vertices().at(f.B());
//		CVertex zc = zeby1->vertices().at(f.C());
//
//		CTriple<double> a(obj->getTransform().w2l(o1->getTransform().l2w(za)));
//		CTriple<double> b(obj->getTransform().w2l(o1->getTransform().l2w(zb)));
//		CTriple<double> c(obj->getTransform().w2l(o1->getTransform().l2w(zc)));
//
//		int rMinX = std::max((int)floor(div * MIN3(a.X(), b.X(), c.X()) - 1.0), sMinX);
//		int rMinY = std::max((int)floor(div * MIN3(a.Y(), b.Y(), c.Y()) - 1.0), sMinY);
//
...
//		CFace f = zeby2->faces().at(i);
//
//		CVertex za = zeby2->vertices().at(f.A());
//		CVertex zb = zeby2->vertices().at(f.B());
//		CVertex zc = zeby2->vertices().at(f.C());
//
//		CTriple<double> a(obj->getTransform().w2l(o2->getTransform().l2w(za)));
//		CTriple<double> b(obj->getTransform().w2l(o2->getTransform().l2w(zb)));
//		CTriple<double> c(obj->getTransform().w2l(o2->getTransform().l2w(zc)));
//
//		int rMinX = std::max((int)floor(div * MIN3(a.X(), b.X(), c.X()) - 1.0), sMinX);
//		int rMinY = std::max((int)floor(div * MIN3(a.Y(), b.Y(), c.Y()) - 1.0), sMinY);
//
...
//
//void CSiateczka1::zrobWyciskSumy2(CModel3D* o1, CModel3D* o2)
//{
//	unsigned long t1 = GetTickCount();
//
//	CMesh* zeby1 = (CMesh*)o1->getData();
...
//	double prog = pMin.Z();
//
//	FILE * plik = fopen("siateczka_bledy.txt", "w");
//
//	for (int iy = sMinY; iy <= sMaxY; iy++)
//	{
//		double y = (double)iy / div;
...
//			double x = (double)ix / div;
//
//			UI::STATUSBAR::printfTimed( 1000, L"Kleję dziury. ( x:%d, y:%d )", ix, iy);
//
//			it = siateczka.find(std::pair<int, int>(ix, iy));
//
//			if (siateczka.end() == it)
//			{
//				std::map< std::pair<int, int>, CVertex* >::iterator left = siateczka.find(std::pair<int, int>(ix - 1, iy));
//				std::map< std::pair<int, int>, CVertex* >::iterator right = siateczka.find(std::pair<int, int>(ix + 1, iy));
//				std::map< std::pair<int, int>, CVertex* >::iterator upper = siateczka.find(std::pair<int, int>(ix, iy + 1));
//				std::map< std::pair<int, int>, CVertex* >::iterator bottom = siateczka.find(std::pair<int, int>(ix, iy - 1));
//
//				double sum = 0.0;
//				int d = 0;
//
//				if ( (siateczka.end() != left) && ((*left).second->Z() > prog) )
//				{
//					sum += (*left).second->Z();
//					d++;
//				}
//
//				if ((siateczka.end() != right) && ((*right).second->Z() > prog) )
//				{
//					sum += (*right).second->Z();
//					d++;
//				}
//
//				if ((siateczka.end() != upper) && ( (*upper).second->Z() > prog) )
//				{
//					sum += (*upper).second->Z();
//					d++;
//				}
//
//				if ((siateczka.end() != bottom) && ((*bottom).second->Z() > prog) )
//				{
//					sum += (*bottom).second->Z();
//					d++;
//				}
...
//				{
//
//					fprintf(plik, "%d, %d : %d : %f : %f\n", ix, iy, d, rzutnia->vertices().at(index).Z(), sum / d);
//
//					rzutnia->vertices().at(index).Z(sum / d);
//
//					//siateczka[std::pair<int, int>(ix, iy)] = &rzutnia->vertices()[index];
//				}
//				else
//				{
//					fprintf(plik, "%d, %d : %d : %f : ---\n", ix, iy, d, rzutnia->vertices().at(index).Z());
//				}
//
//			}
//		}
...
//
//void CSiateczka1::klejDziury2()
//{
//	CMesh * rzutnia = (CMesh*)obj->getData();
//
//	std::map< std::pair<int, int>, CVertex* >::iterator it;
//
//	double prog = pMin.Z() + 0.2;
//
//	FILE * plik = fopen("siateczka_dziury.txt", "w");
//
//	for (int iy = sMinY; iy <= sMaxY; iy++)
...
//			it = siateczka.find(std::pair<int, int>(ix, iy));
//
//			CVertex vI(x, y, prog - 10);
//			CVertex vL(x - 0.1, y, prog - 10);
//			CVertex vR(x + 0.1, y, prog - 10);
//			CVertex vU(x, y + 0.1, prog - 10);
//			CVertex vB(x, y - 0.1, prog - 10);
//
//			if (siateczka.end() != it)
//			{
//				vI = *(*it).second;
//				fprintf(plik, "(%d,%d) %f ", ix, iy, vI.Z());
//			}
//			else
//			{
//				fprintf(plik, "(%d,%d) ---");
...
//			std::map< std::pair<int, int>, CVertex* >::iterator bottom = siateczka.find(std::pair<int, int>(ix, iy - 1));
//
//			if (siateczka.end() != left)
//			{
//				vL = *(*left).second;
//				fprintf(plik, "L: %f ", vL.Z());
//			}
//			else
//			{
//				fprintf(plik, "L: --- ");
//			}
//
//			if (siateczka.end() != right)
//			{
//				vR = *(*right).second;
//				fprintf(plik, "R: %f ", vR.Z());
//			}
//			else
//			{
//				fprintf(plik, "R: --- ");
...
//
//
//			if (siateczka.end() != upper)
//			{
//				vU = *(*upper).second;
//				fprintf(plik, "U: %f ", vU.Z());
//			}
//			else
//			{
//				fprintf(plik, "U: --- ");
//			}
//
//			if (siateczka.end() != bottom)
//			{
//				vB = *(*bottom).second;
//				fprintf(plik, "B: %f ", vB.Z());
//			}
//			else
//			{
//				fprintf(plik, "B: --- ");
//			}
//
//			double srS = (CVector3d(vL, vR).length() + CVector3d(vU, vB).length()) / 2.0;
//			double srI = (CVector3d(vI, vL).length() + CVector3d(vI, vR).length() + CVector3d(vI, vU).length() + CVector3d(vI, vB).length()) / 4;
//
//			if (vI.Z() < prog)
//			{
//				double sum = 0.0;
...
//				if (d > 3)
//				{
//					//fprintf(plik, "%d, %d : %d : %f : %f [%f,%f,%f,%f]\n", ix, iy, d, rzutnia->vertices().at(index).Z(), sum / d, lZ, rZ, uZ, bZ);
//
//					rzutnia->vertices().at(index).Z(sum / d);
//
//					siateczka[std::pair<int, int>(ix, iy)] = &rzutnia->vertices()[index];
//
//					fprintf(plik, "NEW: %f ", sum / d);
//				}
//				else
//				{
//					fprintf(plik, "NEW: ---");
